...
- `wilsjo2-nr` — ещё один форк `dagherbou-dlssnr` (в контрибьюторах тот же
  костяк официального OptiScaler + сам Dagherbou), но поддерживает Neural
  Rendering не только на RTX 50, а на **RTX 20/30/40/50** через технику
  "pre-SR multipass". Каждый релиз сопровождается отдельным `.sha256` от
  автора — лучше проверяемость, чем у dagherbou-dlssnr/klebermotta-mfg.
  Без MFG-unlock.

Все три сторонних форка требуют файл `nvngx_dlssnr.dll` (~165 МБ) — его нет
в архивах, авторы прямо пишут, что распространять его не будут (это файл
...
сканом другой игры), при установке любого из трёх он автоматически
подставится в папку игры — но только если там ещё нет своего (игры с
нативной поддержкой DLSS 5 не трогаются).

Также смотрели `ShyVortex/dlss-unlocked` — не отдельная разработка, а
инсталлятор-сборщик поверх `wilsjo2-nr` + `dlssg_for_sm86` + сторонних
компонентов (DLSS Enabler, DLSSSpoofer, nvapi-dummy, d3d12-proxy — их
отдельно не проверяли). Выглядит легитимно (аккаунт с 2022 года, честные
благодарности апстримам, публичный скрипт сборки), но функционально
дублирует то, что это приложение уже ставит напрямую и прозрачнее — решили
не подключать, вместо этого добавили `wilsjo2-nr` напрямую.
